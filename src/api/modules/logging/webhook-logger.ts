...
export class WebhookLogger {
  async get(channelId: string, name: string) {
    ...
    return webhooks.find(w => w.name === name)
      ?? await channel.createWebhook(name, {
        ...
        reason: `Created for NoCli's webhook logger.`
      });
  }

  async feedback(message: string) {
    const webhook = await this.get(process.env.FEEDBACK_CHANNEL_ID, 'NoCli - Feedback');
    if (!webhook) return;
  ...
  async vote(userId: string, votes: number) {
    const webhook = await this.get(process.env.VOTE_CHANNEL_ID, 'NoCli - Vote');
    if (!webhook) return;
